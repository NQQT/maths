// ─────────────────────────────────────────────────────────────────────────────
// Dashboard plugin HOST components.
//
// These are the mounting surfaces every plugin automatically gets access to
// once registered (see plugins/index.ts and registry.ts):
//
//   <PluginHeaderHost>   — renders the ACTIVE plugin's header component into
//                          the dashboard top bar (right side).
//   <PluginSidebarHost>  — renders the unified left list: every plugin's
//                          entries in registration order, wrapped in shared
//                          list-button chrome (icon + label + active state).
//                          Clicking an entry selects it (activates the owning
//                          plugin's page) via the shared store.
//   <PluginToolbarHost>  — renders the ACTIVE plugin's toolbar component.
//   <PluginPageHost>     — renders the ACTIVE plugin's page component.
//
// All four subscribe to the shared store (useDashboardStore), so selection
// changes and plugin mutations re-render exactly the hosts that care.
// Plugins NEVER import these — the host (MathsDashboard) does. This keeps the
// dependency direction one-way: host → plugin contract ← plugin.
// ─────────────────────────────────────────────────────────────────────────────
...
// ── Sidebar host ──────────────────────────────────────────────────────────────
// The unified left rail. Every plugin contributes its entries here; entries
// keep a stable key of `${pluginId}:${entryId}` so React can track them even
// when plugins come and go. "Coming soon" notices stay plugin-owned: a plugin
// with zero entries simply contributes nothing (the host shows the shared
// empty state when NO plugin has entries at all).

// List button chrome shared by ALL plugins — the visual consistency layer.
// (Mirrors the old TypeSidebar button: icon chip + label, indigo-tinted fill
// when active.)
...
// plugin; mounted when a plugin declares one so future plugins can extend the
// rail without touching the host).
...
export function PluginSidebarHost({ plugins }: PluginHostProps) {
    ...
    // filterEntries (when declared) lets a plugin hide entries that are
    // invalid for its current internal state (e.g. grade-gating); plugins
    // without a filter list all their declared entries.
    const items = plugins.flatMap((plugin) => {
        const entries = plugin.filterEntries
            ? plugin.filterEntries(store.plugins[plugin.id] ?? plugin.initialStore)
            : plugin.entries;
        return entries.map((entry) => ({ plugin, entry }));
    });

    // Selection reconciliation: if the active entry is currently hidden (e.g.
    // the grade no longer offers it), snap the selection to the plugin's
    // first visible entry so the rail highlight and the rendered page can
    // never disagree. Runs in a LAYOUT effect (not during render) — mutating
    // the store mid-render would notify subscribers inside another
    // component's render pass.
    useLayoutEffect(() => {
        const activePlugin = plugins.find((p) => p.id === store.active.pluginId);
        if (!activePlugin) return;
        const visible = items.some(
            (it) => it.plugin.id === activePlugin.id && it.entry.id === store.active.entryId
        );
        const fallback = items.find((it) => it.plugin.id === activePlugin.id);
        if (!visible && fallback) {
            selectEntry(store, activePlugin.id, fallback.entry.id);
        }
    }, [items, plugins, store]);
...
// ── Print host ────────────────────────────────────────────────────────────────
// Mounts the ACTIVE plugin's print surface. The host renders this OUTSIDE
