// ─────────────────────────────────────────────────────────────────────────────
// Dashboard plugin store — the reactive state shared by the host and plugins.
//
...
//     (store.plugins.worksheet.pageCount = 3) bumps the version and notifies
...
// }
//
// Slices are namespaced by plugin id — one plugin can never read or write
// another's slice. forgetPlugin() deletes a slice (and clears the selection
// if it pointed at that plugin), which is what makes plugin deletion leave
// no trace.
// ─────────────────────────────────────────────────────────────────────────────

import React, { createContext, useContext, useRef, useSyncExternalStore } from 'react';
import { isObject } from '@presource/core';
import type { PluginId, PluginStore } from './types';
...
// The pristine store every fresh provider mount starts from.
...
function createCell(initial: DashboardStoreStructure): DashboardCell {
    ...
    // through untouched — the store is only meant to hold plugin data.
...
// The host mounts this exactly once, wrapping every plugin slot.
...
// Hook for host components and plugin slots: subscribes to the cell's
...
// cleared and the host's resolver falls back to another plugin.
